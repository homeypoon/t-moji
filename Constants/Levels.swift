...
struct Levels {
    static let maxPointsPerLevel: [Int: Int] = [
        0: 5, 1: 10, 2: 15, 3: 25, 4: 50, 5: 100, 6: 150, 7: 250,
    ...
    static func getCorrespondingLevelAndMaxPoints(for currentPoints: Int) -> (level: Int, maxPoints: Float) {
        var correspondingLevel = 1
        
        while correspondingLevel < maxPointsPerLevel.count && currentPoints >= maxPointsPerLevel[correspondingLevel]! {
            correspondingLevel += 1
        }
        
        let maxPointsForCorrespondingLevel = maxPointsPerLevel[correspondingLevel - 1]!
        return (level: correspondingLevel, maxPoints: Float(maxPointsForCorrespondingLevel))
    }
